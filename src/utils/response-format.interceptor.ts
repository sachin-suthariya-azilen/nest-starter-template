...
export const SetResponseMessage = (message: string) => SetMetadata('message', message);
...
export class ResponseFormatInterceptor<T> implements NestInterceptor<T, Response<T>> {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<Response<T>> {
    const requestDescription = this.reflector.get<string>('message', context.getHandler());
